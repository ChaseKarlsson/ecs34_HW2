...
class CDSVWriter {
    CDSVWriter(std::shared_ptr< CDataSink > sink, char delimiter, bool quoteall = false);
        ...
        data sink specifies the data destination. By default, only values containing the delimiter, a double quote, or a newline are quoted. When quoteall is set to "true", all values will be quoted.
        for example:
